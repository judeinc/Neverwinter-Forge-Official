...
from id_map_blender_bridge import blender_bridge_status, run_blender_apply_labels_job, run_blender_id_map_job
from id_map_tools import encode_uploaded_texture_underlay, extract_id_map_upload_package
...
class AppHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if route_path in {"/id-map-creator", "/tint-map-maker"}:
            self._send_file(PUBLIC / "id-map-creator.html")
            return
    ...
    def do_POST(self):
        ...
        if self.path == "/api/idmap/extract":
            self._extract_id_map_layout()
            return

        if self.path == "/api/idmap/texture-underlay":
            self._convert_id_map_texture_underlay()
            return

        if self.path == "/api/idmap/suggest":
            self._suggest_id_map_regions()
            return

        if self.path == "/api/idmap/tripo/status":
            self._id_map_tripo_status()
            return

        if self.path == "/api/idmap/tripo/import":
            self._id_map_tripo_import()
            return

        if self.path == "/api/idmap/tripo/task":
            self._id_map_tripo_task()
            return

        if self.path == "/api/idmap/tripo/task-result":
            self._id_map_tripo_task_result()
            return

        if self.path == "/api/idmap/tripo/save-result":
            self._id_map_tripo_save_result()
            return

        if self.path == "/api/idmap/blender/status":
            self._id_map_blender_status()
            return

        if self.path == "/api/idmap/blender/run":
            self._run_id_map_blender_job()
            return

        if self.path == "/api/idmap/blender/analyze-chunks":
            self._analyze_id_map_blender_chunks()
            return

        if self.path == "/api/idmap/blender/apply-labels":
            self._apply_id_map_blender_labels()
            return
    ...
    def _extract_id_map_layout(self):
        try:
            uploaded_files = self._read_multipart_files(max_bytes=300 * 1024 * 1024)
            result = extract_id_map_upload_package(uploaded_files)
        except Exception as exc:
            self._send_json({"error": str(exc)}, status=400)
            return
    ...
    def _convert_id_map_texture_underlay(self):
        try:
            uploaded_files = self._read_multipart_files(max_bytes=80 * 1024 * 1024)
            texture_files = [
                item for item in uploaded_files
                if Path(item.get("name", "")).suffix.lower() in {".dds", ".png", ".jpg", ".jpeg", ".webp", ".bmp", ".tga"}
            ]
            if not texture_files:
                raise ValueError("Drop or choose one texture image.")
            underlay = encode_uploaded_texture_underlay(texture_files[0])
            if not underlay:
                raise ValueError("Could not read that texture image.")
        except Exception as exc:
            self._send_json({"error": str(exc)}, status=400)
            return

        self._send_json({"underlay": underlay})

    def _suggest_id_map_regions(self):
        try:
            payload = self._read_json(max_bytes=24 * 1024 * 1024)
            api_key = str(payload.get("openaiApiKey", "")).strip()
            if not api_key:
                raise ValueError("Enter an OpenAI API key for the AI pre-pass.")
            image_data = str(payload.get("imageData", "")).strip()
            if not image_data:
                raise ValueError("No UV reference image was provided.")
            mime_type = str(payload.get("mimeType", "image/png")).strip() or "image/png"
            prompt = str(payload.get("prompt", "")).strip()
            if not prompt:
                raise ValueError("No ID map pre-pass prompt was provided.")
            model = str(payload.get("openaiModel", "gpt-image-2")).strip() or "gpt-image-2"
            result = call_openai_image_edit(
                api_key,
                model,
                prompt,
                image_data,
                mime_type,
                {
                    "openaiSize": str(payload.get("openaiSize", "1024x1024")),
                    "openaiQuality": str(payload.get("openaiQuality", "medium")),
    ...
    def _id_map_tripo_status(self):
        try:
            payload = self._read_json(max_bytes=1024 * 1024)
            api_key = str(payload.get("tripoApiKey", "")).strip()
            if not api_key:
                raise ValueError("Enter a Tripo API key.")
            result = check_tripo_balance(api_key)
        except Exception as exc:
            self._send_json({"error": str(exc)}, status=400)
            return
    ...
    def _id_map_tripo_import(self):
        try:
            form = self._read_multipart_form(max_bytes=180 * 1024 * 1024)
            api_key = str(form["fields"].get("tripoApiKey", "")).strip()
            if not api_key:
                raise ValueError("Enter a Tripo API key.")
            model_file = next(
                (item for item in form["files"] if Path(uploaded_file_name(item)).suffix.lower() in {".obj", ".fbx", ".glb", ".stl"}),
                None
            )
            if not model_file:
                raise ValueError("Tripo import currently accepts OBJ, FBX, GLB, or STL model files.")
            options = {}
            raw_options = form["fields"].get("options", "")
            if raw_options:
                options = json.loads(raw_options)
            result = import_tripo_model(api_key, model_file, options)
        except Exception as exc:
            self._send_json({"error": str(exc)}, status=400)
            return
    ...
    def _id_map_tripo_task(self):
        try:
            ...
            if not isinstance(task_payload, dict) or not task_payload.get("type"):
                raise ValueError("Tripo task payload must include a type.")
            if task_payload.get("type") == "texture_model":
                attach_tripo_texture_reference_images(api_key, task_payload, payload.get("textureReferenceImages", []))
            result = create_tripo_task(api_key, task_payload)
        except Exception as exc:
            self._send_json({"error": str(exc)}, status=400)
            return
    ...
    def _id_map_tripo_task_result(self):
        try:
            payload = self._read_json(max_bytes=1024 * 1024)
            api_key = str(payload.get("tripoApiKey", "")).strip()
            ...
            if not api_key:
                raise ValueError("Enter a Tripo API key.")
            if not task_id:
                raise ValueError("Enter a Tripo task id.")
            result = get_tripo_task(api_key, task_id)
        except Exception as exc:
            self._send_json({"error": str(exc)}, status=400)
            return
    ...
    def _id_map_tripo_save_result(self):
        try:
            payload = self._read_json(max_bytes=10 * 1024 * 1024)
            result = payload.get("result", {})
            if not isinstance(result, dict):
                raise ValueError("No Tripo result JSON was provided.")
            saved = save_tripo_result_assets(result)
        except Exception as exc:
            self._send_json({"error": str(exc)}, status=400)
            return
    ...
    def _id_map_blender_status(self):
        try:
            payload = self._read_json(max_bytes=1024 * 1024)
            result = blender_bridge_status(ROOT, payload.get("blenderPath", ""))
        except Exception as exc:
            self._send_json({"error": str(exc)}, status=400)
            return
    ...
    def _run_id_map_blender_job(self):
        try:
            form = self._read_multipart_form(max_bytes=350 * 1024 * 1024)
            options = {}
            raw_options = form["fields"].get("options", "")
            if raw_options:
                options = json.loads(raw_options)
            result = run_blender_id_map_job(ROOT, form["files"], options)
        except Exception as exc:
            self._send_json({"error": str(exc)}, status=400)
            return
    ...
    def _analyze_id_map_blender_chunks(self):
        try:
            payload = self._read_json(max_bytes=32 * 1024 * 1024)
            api_key = str(payload.get("openaiApiKey", "")).strip()
            if not api_key:
                raise ValueError("Enter an OpenAI API key for 3D chunk analysis.")
            model = str(payload.get("openaiVisionModel", "gpt-5.5")).strip() or "gpt-5.5"
            images = payload.get("images", [])
            chunk_lookup = payload.get("chunkLookup", {})
            if not images:
                raise ValueError("Run Blender 3D Bake first so texture and chunk-ID helper views are available.")
            result = call_openai_chunk_label_analysis(api_key, model, images, chunk_lookup, payload)
        except Exception as exc:
            self._send_json({"error": str(exc)}, status=400)
            return
    ...
    def _apply_id_map_blender_labels(self):
        try:
            payload = self._read_json(max_bytes=8 * 1024 * 1024)
            job_dir = str(payload.get("jobDir", "")).strip()
            if not job_dir:
                raise ValueError("Run Blender 3D Bake before applying AI labels.")
            analysis = payload.get("analysis", {})
            labels = analysis.get("chunkLabels", []) if isinstance(analysis, dict) else payload.get("chunkLabels", [])
            if not labels:
                raise ValueError("Analyze 3D chunks before applying labels.")
            options = {
                "blenderPath": payload.get("blenderPath", ""),
                "textureSize": payload.get("textureSize", 1024),
                "timeoutSeconds": payload.get("timeoutSeconds", 600),
            }
            result = run_blender_apply_labels_job(ROOT, job_dir, labels, options)
        except Exception as exc:
            self._send_json({"error": str(exc)}, status=400)
            return
...
def call_openai_chunk_label_analysis(api_key, model, images, chunk_lookup, payload):
    chunk_rows = []
    for chunk in (chunk_lookup or {}).get("chunks", [])[:320]:
        chunk_rows.append({
            "chunkId": chunk.get("id"),
            "colorHex": chunk.get("colorHex"),
            "faceCount": chunk.get("faceCount"),
            "grid": chunk.get("grid"),
        })

    prompt = {
        "task": "Analyze paired 3D model renders for a Neverwinter Nights 2 tint/ID map workflow.",
        "important_context": [
            "The UV unwrap may be chaotic and should not be used for visual reasoning.",
            "Texture Reference images show the model with its diffuse/albedo texture wrapped in 3D.",
            "Chunk ID images are matching camera views where each flat color represents a mesh-space chunk.",
            "Mesh Wire images show the same textured model with rendered triangle/quad flow so cuffs, hems, gloves, boots, trim, and decimation-followed curves can be judged by surface topology.",
            "Normals images show surface-normal changes and curvature, which can reveal object cutoffs even when the UV unwrap or spatial chunk grid crosses several garment parts.",
            "Use the Texture Reference image to understand the garment/object part, then use the Chunk ID image to identify which colored chunks belong to that part.",
            "Use Mesh Wire and Normals images as helper evidence for cut lines, curved cuffs, robe hems, bracer ends, boot tops, collars, and hard surface changes.",
            "Be broad and practical. Do not classify tiny seams, stitches, scratches, rivets, or noise as separate regions."
        ],
        "allowed_labels": [
            "tunic_top",
            "sleeves",
        ...
        "output_json_schema": {
            ...
            "chunkLabels": [
                {
                    "chunkId": "one of the provided chunkIds when confident, otherwise empty",
                    "colorHex": "matching chunk color when visible",
            ...
            "regionNotes": [
                {"label": "allowed label", "notes": "what the model part appears to be"}
            ],
            "warnings": ["ambiguities or failure cases"]
        },
        "chunks": chunk_rows,
    }
    ...
    content = [
        {
            ...
            "text": (
                ...
                "Use only the allowed labels. Prefer uncertain when chunk identity is unclear.\n\n"
                + json.dumps(prompt, indent=2)
            ),
        }
    ]
    for item in images[:20]:
        image_data = str(item.get("imageData", "")).strip()
        mime_type = str(item.get("mimeType", "image/png")).strip() or "image/png"
        name = str(item.get("name", item.get("kind", "image")))
        kind = str(item.get("kind", "reference"))
        if not image_data:
            continue
        content.append({"type": "text", "text": f"Image: {kind} / {name}"})
        content.append({
            "type": "image_url",
    ...
    try:
        ...
        detail = exc.read().decode("utf-8", errors="replace")
        raise RuntimeError(f"OpenAI chunk analysis error {exc.code}: {detail}") from exc

    raw_text = response_body.get("choices", [{}])[0].get("message", {}).get("content", "")
    analysis = parse_json_from_model_text(raw_text)
    return {
        "model": model,
        "analysis": analysis,
        "rawText": raw_text,
...
def parse_json_from_model_text(text):
    ...
    if not text:
        return {"summary": "No analysis was returned.", "chunkLabels": [], "warnings": ["Empty model response."]}
    try:
...
def check_tripo_balance(api_key):
    req = request.Request(
        f"{TRIPO_API_BASE}/user/balance",
        method="GET",
        headers={
            "Authorization": f"Bearer {api_key}",
    ...
    try:
        ...
        try:
            ...
            details = {}
        message = details.get("message") or details.get("error") or f"Tripo rejected the key with HTTP {exc.code}."
        raise RuntimeError(message) from exc
    except error.URLError as exc:
        raise RuntimeError(f"Could not reach Tripo API: {exc.reason}") from exc

    data = payload.get("data", payload) if isinstance(payload, dict) else {}
    if not isinstance(data, dict):
        data = {}
    balance = data.get("balance", data.get("remaining_balance", data.get("credits", "")))
    frozen = data.get("frozen", data.get("frozen_balance", ""))
    return {
        "ok": True,
        "balance": balance,
        "frozen": frozen,
        "rawCode": payload.get("code") if isinstance(payload, dict) else None,
    }


def uploaded_file_name(model_file):
    return model_file.get("filename") or model_file.get("name") or "model.obj"


def uploaded_file_bytes(model_file):
    return model_file.get("content") or model_file.get("data") or b""


def import_tripo_model(api_key, model_file, options=None):
    options = options or {}
    filename = Path(uploaded_file_name(model_file)).name
    suffix = Path(filename).suffix.lower().lstrip(".")
    if suffix not in {"obj", "fbx", "glb", "stl"}:
        raise ValueError("Tripo import currently accepts OBJ, FBX, GLB, or STL model files.")
    content = uploaded_file_bytes(model_file)
    if not content:
        raise ValueError("The selected model file is empty.")
    upload = upload_tripo_object(api_key, filename, content, model_file.get("mime_type") or model_file.get("mimeType") or mimetypes.guess_type(filename)[0] or "application/octet-stream")
    object_file = {
        "object": {
    ...
    if isinstance(options.get("payload"), dict):
        payload = deep_merge_dicts(payload, options["payload"])
    result = create_tripo_task(api_key, payload)
    result["upload"] = {
        "bucket": upload["bucket"],
        "key": upload["key"],
        "filename": filename,
...
def attach_tripo_texture_reference_images(api_key, task_payload, reference_images):
    if not isinstance(reference_images, list) or not reference_images:
        return task_payload

    uploaded_images = []
    for index, image in enumerate(reference_images[:2], start=1):
        if not isinstance(image, dict):
            continue
        image_data = str(image.get("imageData") or image.get("base64") or "").strip()
        if not image_data:
            continue
        mime_type = str(image.get("mimeType", "image/png")).split(";")[0] or "image/png"
        if not mime_type.startswith("image/"):
            raise ValueError("Tripo texture references must be image files.")
        filename = safe_slug(str(image.get("name") or image.get("kind") or f"reference-{index}"))
        suffix = extension_for_upload(mime_type)
        content = base64.b64decode(image_data)
        upload = upload_tripo_object(api_key, filename + suffix, content, mime_type)
    ...
    prompt = task_payload.get("texture_prompt")
    if not isinstance(prompt, dict):
        prompt = {}
    prompt.pop("text", None)
    prompt.pop("image", None)
    prompt["images"] = uploaded_images
    task_payload["texture_prompt"] = prompt
    return task_payload


def create_tripo_task(api_key, payload):
    return tripo_api_json(api_key, "POST", "/task", payload)


def get_tripo_task(api_key, task_id):
    return tripo_api_json(api_key, "GET", f"/task/{quote(task_id, safe='')}")


def upload_tripo_object(api_key, filename, content, mime_type):
    suffix = Path(filename).suffix.lower().lstrip(".") or "obj"
    sts = get_tripo_upload_sts(api_key, suffix)
    upload_url = first_dict_value(sts, "upload_url", "url", "put_url", "presigned_url")
    bucket = first_dict_value(sts, "bucket", "resource_bucket")
    key = first_dict_value(sts, "key", "resource_uri", "resource_key", "object_key")
    if upload_url:
        put_tripo_file(upload_url, content, mime_type, {})
    else:
        access_key = first_dict_value(sts, "access_key_id", "AccessKeyId", "ak", "tmp_ak", "sts_ak")
        secret_key = first_dict_value(sts, "secret_access_key", "SecretAccessKey", "sk", "tmp_sk", "sts_sk")
        session_token = first_dict_value(sts, "session_token", "SessionToken", "token", "tmp_token", "sts_token", "security_token")
        region = first_dict_value(sts, "region", "Region") or region_from_s3_host(first_dict_value(sts, "s3_host", "host")) or "us-west-2"
        if not bucket or not key or not access_key or not secret_key:
            raise RuntimeError(f"Tripo STS response did not include enough upload details: {json.dumps(sts)[:800]}")
        upload_url = build_s3_upload_url(sts, bucket, key, region)
        put_tripo_file_with_sigv4(upload_url, content, mime_type, access_key, secret_key, session_token, region)
    ...
    if not bucket or not key:
        raise RuntimeError("Tripo upload succeeded but bucket/key could not be determined.")
    return {"bucket": bucket, "key": key}


def get_tripo_upload_sts(api_key, file_format):
    payload_options = [
        {"format": file_format},
        {"file_format": file_format},
        {"type": file_format},
    ]
    paths = ["/upload/sts/token", "/upload/sts"]
    last_error = None
    for path in paths:
        for payload in payload_options:
            try:
                result = tripo_api_json(api_key, "POST", path, payload)
                data = result.get("data", result)
                if isinstance(data, dict):
                    return data
            except Exception as exc:
                last_error = exc
    raise RuntimeError(f"Could not get a Tripo upload token: {last_error}")


def tripo_api_json(api_key, method, path, payload=None):
    body = None
    headers = {
        "Authorization": f"Bearer {api_key}",
        "Accept": "application/json",
    ...
    req = request.Request(f"{TRIPO_API_BASE}{path}", data=body, method=method, headers=headers)
    try:
        with request.urlopen(req, timeout=60) as response:
            response_text = response.read().decode("utf-8")
    except error.HTTPError as exc:
        detail = exc.read().decode("utf-8", errors="replace")
        raise RuntimeError(format_tripo_error(exc.code, detail)) from exc
    except error.URLError as exc:
        raise RuntimeError(f"Could not reach Tripo API: {exc.reason}") from exc
    try:
...
def format_tripo_error(status, detail):
    try:
    ...
    if isinstance(parsed, dict):
        ...
        if message:
            ...
            if status == 403 and "credit" in lowered:
                return f"Tripo credit issue ({status}): {message}. Add API credits or upgrade API access in Tripo, then retry this task."
            return f"Tripo API error ({status}): {message}"
    return f"Tripo API error ({status}): {detail[:1200] or 'no response body'}"


def put_tripo_file(upload_url, content, mime_type, headers):
    request_headers = dict(headers or {})
    if "content-type" not in {key.lower() for key in request_headers}:
        request_headers["Content-Type"] = mime_type
    req = request.Request(upload_url, data=content, method="PUT", headers=request_headers)
    try:
        with request.urlopen(req, timeout=120):
            return True
    except error.HTTPError as exc:
        detail = exc.read().decode("utf-8", errors="replace")
        raise RuntimeError(f"Tripo object upload failed ({exc.code}): {detail[:1200] or exc.reason}") from exc
    except error.URLError as exc:
        raise RuntimeError(f"Could not upload object to Tripo storage: {exc.reason}") from exc


def put_tripo_file_with_sigv4(upload_url, content, mime_type, access_key, secret_key, session_token, region):
    parsed = urlparse(upload_url)
    host = parsed.netloc
    path = parsed.path or "/"
    now = datetime.utcnow()
    ...
    signing_key = aws_sigv4_key(secret_key, date_stamp, region, "s3")
    ...
    put_tripo_file(upload_url, content, mime_type, headers)
...
def build_s3_upload_url(sts, bucket, key, region):
    endpoint = first_dict_value(sts, "endpoint", "host", "s3_endpoint", "s3_host")
    if endpoint:
        ...
        if parsed.netloc.startswith("s3.") or parsed.netloc == "s3.amazonaws.com":
            return f"{endpoint.rstrip('/')}/{bucket}/{quote(key, safe='/~')}"
        if bucket not in parsed.netloc.split("."):
            return f"https://{bucket}.{parsed.netloc}/{quote(key, safe='/~')}"
        return f"{endpoint.rstrip('/')}/{quote(key, safe='/~')}"
    return f"https://{bucket}.s3.{region}.amazonaws.com/{quote(key, safe='/~')}"


def region_from_s3_host(host):
    host = str(host or "")
    parts = host.split(".")
...
def first_dict_value(source, *keys):
...
def deep_merge_dicts(target, source):
    for key, value in source.items():
        if isinstance(value, dict) and isinstance(target.get(key), dict):
            target[key] = deep_merge_dicts(dict(target[key]), value)
        else:
            target[key] = value
    return target


def save_tripo_result_assets(result):
    data = result.get("data", result)
    if not isinstance(data, dict):
        raise ValueError("Tripo result did not include a data object.")
    task_id = str(data.get("task_id") or data.get("taskId") or result.get("task_id") or "tripo-result").strip()
    task_slug = safe_slug(task_id) or f"tripo-{uuid.uuid4().hex[:8]}"
    output = data.get("output", {}) if isinstance(data.get("output"), dict) else {}
    nested = data.get("result", {}) if isinstance(data.get("result"), dict) else {}
    model_url = output.get("model") or (nested.get("model") or {}).get("url") or data.get("model_url")
    preview_url = output.get("rendered_image") or data.get("thumbnail") or (nested.get("rendered_image") or {}).get("url")
    if not model_url and not preview_url:
        raise ValueError("This Tripo result does not include downloadable model or preview URLs yet.")

    target_dir = OUTPUTS / "id-map-creator" / "tripo" / task_slug
    target_dir.mkdir(parents=True, exist_ok=True)
    saved_assets = []
    if preview_url:
        saved_assets.append(download_tripo_asset(preview_url, target_dir, f"{task_slug}_preview", {".webp", ".png", ".jpg", ".jpeg"}))
    if model_url:
        saved_assets.append(download_tripo_asset(model_url, target_dir, f"{task_slug}_textured_model", {".glb", ".gltf", ".obj", ".fbx", ".zip"}))

    manifest = {
        "taskId": task_id,
        "savedAt": datetime.utcnow().isoformat(timespec="seconds") + "Z",
        "consumedCredit": data.get("consumed_credit"),
...
def download_tripo_asset(url, target_dir, base_name, allowed_exts):
    parsed = urlparse(str(url))
    ext = Path(parsed.path).suffix.lower()
    if ext not in allowed_exts:
        ext = next(iter(sorted(allowed_exts)))
    filename = safe_slug(base_name) + ext
    target = target_dir / filename
    req = request.Request(str(url), headers={"User-Agent": "Neverwinter Forge/1.0"})
    try:
        with request.urlopen(req, timeout=180) as response:
            content = response.read()
            mime_type = response.headers.get("Content-Type", mimetypes.guess_type(filename)[0] or "application/octet-stream")
    except error.HTTPError as exc:
        detail = exc.read().decode("utf-8", errors="replace")
        raise RuntimeError(f"Could not download Tripo asset ({exc.code}): {detail[:500] or exc.reason}") from exc
    except error.URLError as exc:
        raise RuntimeError(f"Could not download Tripo asset: {exc.reason}") from exc
    target.write_bytes(content)
    relative = target.relative_to(OUTPUTS).as_posix()
    return {
        "name": filename,
        "path": str(target),
