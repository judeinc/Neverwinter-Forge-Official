...
### Added
- Added an ID Map Creator / Tint Map Maker Forge tool at `/id-map-creator` and `/tint-map-maker` for creating hard-edged color ID maps from MDB UV layouts.
- Added MDB UV extraction support for the ID Map Creator, including RIGD/SKIN UV triangles, UV island grouping, material texture references, and NWN2 negative-V normalization.
- Added a browser canvas ID map workflow with diffuse/normal texture underlays, exact-color palette assignment, UV wire/ID overlay toggles, undo/redo, auto-color islands, and hard-edged PNG export.
- Added an ID Map Creator AI pre-pass prompt asset and copy action for broad tintable region suggestions such as tunic, pants, boots, gloves, belts, armor, leather panels, and large accessories.
- Added ZIP package support for ID Map Creator imports so an MDB plus matching DDS diffuse/normal textures can be dropped together and converted into browser underlays automatically.
- Added separate Diffuse / Albedo and Normal Map drop boxes for loose ID Map Creator texture underlays, including DDS/TGA conversion through the local Forge backend.
- Added an AI Suggest Regions action for ID Map Creator that sends a UV/diffuse/normal reference to OpenAI, then snaps the returned suggestion back onto actual UV islands using the approved exact-color ID palette.
- Added OBJ and best-effort ASCII FBX UV imports to the ID Map Creator, and made the central UV canvas accept model drops in addition to the top model drop zone.
- Increased the ID Map Creator model upload cap and added high-detail OBJ/FBX browser proxy reduction so Substance-style high-poly exports can load without sending millions of UV triangles to the canvas.
- Added an advanced Blender 3D Bake bridge for ID Map Creator that launches Blender in background mode, imports OBJ/FBX meshes, renders multi-view surface-color references, and bakes a starter geometric ID map through the model UVs.
- Added Blender 3D helper passes that render the model with its supplied diffuse/albedo texture plus matching chunk-ID views, giving future AI labeling a 3D visual reference instead of relying on chaotic UV unwraps.
- Added an AI Analyze 3D Chunks pass that sends Blender texture/chunk-ID helper views to an OpenAI vision model and returns reviewable broad-region labels before any automatic rebake step.
- Added an Apply AI Labels Blender pass that maps reviewed chunk labels back onto mesh chunks and bakes an AI-labeled ID map through the model UVs.
- Added editable 3D chunk review controls so users can correct AI labels and confidence values before baking the final AI-labeled ID map.
- Added Blender mesh-wire and surface-normal helper renders for the ID Map Creator so AI chunk analysis can use topology, curve flow, cuffs, hems, boot tops, bracer ends, and normal cutoffs as extra evidence.
- Added exact-palette postprocessing for Blender-baked ID maps so final PNG colors snap back to approved hard ID colors instead of Blender gamma/anti-aliased near-colors.
- Organized the ID Map Creator into workflow methods, staging Tripo AI Segmentation as the next primary path while keeping Local Blender Helper Bake and UV Island / Image Pass available as fallback workflows.
- Added a 3D/UV view toggle to ID Map Creator, with an OBJ 3D preview stage for the Tripo texture-generation workflow while preserving the UV canvas for fallback/manual review.
- Fixed the ID Map Creator OBJ 3D preview to recompute display normals and correct inward triangle winding instead of trusting imported OBJ normals.
- Added an ID Map Creator 3D preview normals selector with two-sided, OBJ, auto, and flip modes, and render OBJ previews in right-handed space for more reliable model inspection.
- Adjusted the ID Map Creator 3D preview's default camera to open on the model front after switching OBJ previews to right-handed rendering.
- Expanded the ID Map Creator Tripo AI workflow with import, mesh segmentation, texture generation, task refresh, model/task id tracking, prompt controls, and advanced JSON overrides for experimentation.
- Fixed Tripo model import so the Forge backend recognizes uploaded OBJ/FBX/GLB/STL files from the shared multipart parser.
- Clarified Tripo credit-related 403 errors in the ID Map Creator so exhausted API credits are reported plainly.
- Added Tripo task auto-polling and output cards for completed rendered previews and GLB downloads in the ID Map Creator.
- Added a Save Result action for ID Map Creator Tripo outputs that downloads completed GLB and preview assets into local Forge outputs and links them back into the UI.
- Added explicit Tripo output actions for opening saved assets and previewing completed GLB results in the ID Map Creator 3D viewport.
- Added Tripo prompt recipes and target-zone checkboxes for generating flat color segmentation masks, material blockouts, or readable texture previews without game-specific prompt wording.
- Changed Tripo GLB preview to save remote Tripo outputs locally before loading them in Babylon, avoiding remote signed-URL loader failures.
- Tightened Tripo flat ID mask experiments with exact hex color assignments, non-PBR geometry-aligned texture requests, and calmer 3D preview orbit controls with double-click camera reset.
- Added an AI Forge ID Map Color Guide preset with a paired front/back source-and-output workbench, recoloring outfit renders into flat exact-palette source guides for texture projection.
- Added front/back Tripo reference image slots to the ID Map Creator, uploading those guides into Tripo `texture_prompt.images` for image-guided texture experiments.
- Improved paired ID Map Color Guide generation so the back pass can use the generated front guide as an approved color-label reference, with tighter collar/gorget/chainmail consistency rules.
- Tightened the ID Map Color Guide prompt for low-quality tests so waist belts, diagonal straps, metal greaves, and navy boot regions stay separated instead of merging into nearby clothing.
